...
| 35 | The calendar scrolls itself into view | Add sheet, tap the date row | The calendar fades in and the form scrolls until the whole of it is on screen | Pass |
| 36 | The compact title appears | Expenses tab, scroll down | The large title travels up and a small "Expenses" title fades in at the top | Pass |
| 37 | The segmented thumb travels | Stats, tap Share | The white thumb slides from Categories to Share rather than jumping | Pass |
| 38 | A day is one card | Expenses tab with two or more expenses on a day | The day's rows share one rounded card, divided by hairlines that start at the label | Pass |
...
| The note field on the Add sheet did not scroll clear of the keyboard | The sheet is barely taller than its content, so before the keyboard appears there is almost nothing to scroll and the request was clamped to zero. The room only exists once `automaticallyAdjustKeyboardInsets` has extended the content, which happens after the focus event | The field is revealed again on `keyboardDidShow`, once the room exists. **Not confirmed on a device.** The simulator kept the hardware keyboard attached and would not show the software keyboard on this screen, and the second simulator was not available. The Settings field in test 34 covers the same code on a page that can already scroll. Worth one check by hand on the phone |
| The compact title bar passed taps through to the rows it covered | The bar is drawn over the list and was marked `pointerEvents="none"` so it would not block the list while it was invisible. Once it faded in it stayed transparent to touch, so a tap on it reached whichever row had scrolled underneath | The bar now takes its own taps while it is solid and lets them through while it is not. Confirmed on the Settings screen, which is long enough to hide a currency row behind the bar — see test 41 |
